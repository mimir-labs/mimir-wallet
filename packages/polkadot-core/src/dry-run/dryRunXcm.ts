...
export async function dryRunWithXcm(
  ...
  for (let i = 0; i < forwardedXcms.length; i++) {
    ...
    try {
      const { chainApi, originInfo } = await processXcmLocation(xcm[0], {
        isSupport: true,
        ...initialChain,
      });
      ...
      const xcmResults = await executeXcmMessages(
        chainApi,
        xcm[1],
        originParents,
        originInterior,
...
async function processXcmLocation(
  ...
  const chainApi: ApiPromise = await ApiManager.getInstance().getApi(
    chain.genesisHash,
  );
...
async function executeXcmMessages(
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    try {
      ...
      results.push(
        createErrorResult(error, 'Unknown error during XCM execution'),
      );
    }
  }
  ...
  return {
    ...
    error:
      error instanceof Error
        ? error
        : new Error(`${defaultMessage}: ${String(error)}`),
  };
}
