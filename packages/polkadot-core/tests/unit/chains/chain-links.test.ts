...
describe('chainLinks', () => {
  describe('accountExplorerLink', () => {
    it('should generate explorer URL with valid SS58 address', () => {
      const link = chainLinks.accountExplorerLink(chainWithExplorer, ALICE_SS58);

      expect(link).toBe('https://polkadot.subscan.io/account/15oF4uVJwmo4TdGW7VfQxNLavjCXviqxT9S1MgbjMNHr6Sp5');
    });
    ...
    it('should use statescan URL when explorer URL is not available', () => {
      const link = chainLinks.accountExplorerLink(chainWithStatescan, ALICE_SS58);

      expect(link).toBe('https://polkadot.statescan.io/#/accounts/15oF4uVJwmo4TdGW7VfQxNLavjCXviqxT9S1MgbjMNHr6Sp5');
    });

    it('should prefer explorer URL over statescan URL', () => {
      const link = chainLinks.accountExplorerLink(chainWithBothUrls, ALICE_SS58);
    ...
    it('should encode address with correct ss58Format', () => {
      const link = chainLinks.accountExplorerLink(chainWithSs58Format42, ALICE_HEX);

      // With ss58Format 42, the address should be different from format 0
      expect(link).toContain('5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY');
    });
  ...
  describe('extrinsicExplorerLink', () => {
    const extrinsicHash = '0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef';
    const blockExtrinsic = '12345-2';

    it('should generate explorer URL for extrinsic hash', () => {
      const link = chainLinks.extrinsicExplorerLink(chainWithExplorer, extrinsicHash);

      expect(link).toBe(`https://polkadot.subscan.io/extrinsic/${extrinsicHash}`);
    });

    it('should generate explorer URL for block-extrinsic format', () => {
      const link = chainLinks.extrinsicExplorerLink(chainWithExplorer, blockExtrinsic);

      expect(link).toBe(`https://polkadot.subscan.io/extrinsic/${blockExtrinsic}`);
    });

    it('should use statescan URL when explorer URL is not available', () => {
      const link = chainLinks.extrinsicExplorerLink(chainWithStatescan, extrinsicHash);

      expect(link).toBe(`https://polkadot.statescan.io/#/extrinsics/${extrinsicHash}`);
    });

    it('should prefer explorer URL over statescan URL', () => {
      const link = chainLinks.extrinsicExplorerLink(chainWithBothUrls, extrinsicHash);

      expect(link).toBe(`https://polkadot.subscan.io/extrinsic/${extrinsicHash}`);
    });

    it('should return undefined when no explorer URLs are available', () => {
      const link = chainLinks.extrinsicExplorerLink(chainWithNoUrls as any, extrinsicHash);
    ...
    it('should handle object with toString method', () => {
      ...
      const link = chainLinks.extrinsicExplorerLink(chainWithExplorer, extrinsicObj);

      expect(link).toBe('https://polkadot.subscan.io/extrinsic/67890-5');
    });

    it('should handle undefined value', () => {
      const link = chainLinks.extrinsicExplorerLink(chainWithExplorer, undefined);
