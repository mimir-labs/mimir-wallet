...
import { Avatar, Box, Button, IconButton, Paper, Stack, SvgIcon, Typography, useMediaQuery, useTheme } from '@mui/material';
...
function Row({
  ...
  const format = useMemo((): [decimals: number, unit: string] => [decimals || api.registry.chainDecimals[0], symbol || tokenSymbol], [api, decimals, symbol, tokenSymbol]);
  ...
  const RowMain = () => (
    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 1 }}>
      ...
      <Button component={Link} endIcon={downSm ? undefined : <SvgIcon component={IconSend} inheritViewBox />} sx={{ minWidth: downSm ? 0 : undefined }} to={`/transfer?assetId=${assetId}`}>
  ...
  const RowSub = () => (
    <Paper sx={{ width: '100%', marginTop: { sm: 2, xs: 1.5 }, bgcolor: 'secondary.main', borderRadius: 2, padding: 2 }}>
      <Box
        sx={{
          display: 'flex',
