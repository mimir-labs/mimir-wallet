...
function SwitchNetworks({ networks }: SwitchNetworksProps) {
  ...
  const networkList = useMemo(() => {
    ...
    for (const networkItem of networks) {
      const networkDetails = allNetworks.find(
        (network) => network.key === networkItem.networkKey,
      );
  ...
  return networkList.map(({ key, ...network }) => (
    <NetworkItem key={key} {...network} />
  ));
}
...
function NetworkItem(network: {
  ...
  return (
    <div className="border-divider flex w-full items-center justify-between rounded-[10px] border p-[10px]">
      ...
      {/* Right side: Status */}
      <div
        className={`text-[14px] font-bold ${isEnabled ? 'text-success' : 'text-danger'}`}
      >
