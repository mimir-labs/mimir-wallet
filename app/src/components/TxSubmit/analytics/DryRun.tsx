...
function OperationItemContent({
  ...
  return (
    <div className="bg-secondary flex items-center justify-between rounded-[5px] p-[5px] text-xs">
      ...
      {assetId === 'native' ? (
        <NativeToken amount={amount} />
      ) : (
        <AssetToken assetId={assetId} amount={amount} />
      )}
    </div>
  );
}
...
type SelfBalanceChange = {
  genesisHash: HexString;
  assetId: string | 'native';
  amount: bigint;
};

function extraBalancesChange(
  address: string,
  ...
  for (const change of balancesChanges) {
    if (addressEq(change.from, address)) {
      const exists = results.find(
        (item) =>
          item.genesisHash === change.genesisHash &&
          item.assetId === change.assetId,
      );
...
function DryRun({ call, account }: { call: IMethod; account?: string }) {
            ...
            try {
              ...
              if (errorResult) {
                ...
                setBalancesChanges(
                  extraBalancesChange(
                    account,
                    balancesChanges.concat(
                      xcmResult
                        .map((item) =>
                          item.success ? item.balancesChanges : [],
                        )
                        .flat(),
                    ),
                  ),
  ...
  if (simulation.isDone && !simulation.success) {
    return (
      <DryRunFailed
        title="Simulation Result: Failed!"
  ...
  if (simulation.isLoading || crossChainSimulation.isLoading) {
    return (
      <DryRunPending
        title={
          simulation.isLoading ? 'Simulating...' : 'Cross-Chain Simulating...'
        }
      />
    );
  }
  ...
  if (simulation.isDone && simulation.success) {
    if (crossChainSimulation.isDone && !crossChainSimulation.success) {
      return (
        <DryRunFailed
          title="Cross-Chain Simulation Result: Failed!"
          error={crossChainSimulation.error}
        />
      );
    }
