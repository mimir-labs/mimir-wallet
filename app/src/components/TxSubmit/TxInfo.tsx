    ...
    <div className='flex items-center gap-2.5'>
      ...
      <div className='flex flex-1 flex-col gap-[5px]'>
        <div className='flex flex-wrap items-center gap-[5px]'>
          ...
          <div className='bg-secondary flex items-center gap-[5px] rounded-[10px] px-2.5 py-1'>
            <Avatar src={chain.icon} alt={chain.name} style={{ width: 16, height: 16, background: 'transparent' }} />
            <span className='text-foreground text-sm font-bold'>{chain.name}</span>
          </div>
          {(!!website || !!appName) && !website?.startsWith('mimir://internal') && (
            <>
              <span className='text-primary font-bold'>via</span>
              <div className='bg-secondary flex items-center rounded-[10px] px-2.5 py-1 font-bold'>
        ...
        <div
          className='text-foreground/50 hover:text-primary flex cursor-pointer items-center gap-[5px] text-xs transition-colors'
          onClick={() => copyAddress()}
        >
          <span>
