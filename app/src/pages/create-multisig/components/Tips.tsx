  ...
  const remoteProxyChain = useMemo(
    () =>
      remoteProxyRelations[chain.genesisHash]
        ? allEndpoints.find(
            (item) =>
              item.genesisHash === remoteProxyRelations[chain.genesisHash],
          )
        : null,
    [chain.genesisHash],
  );
      ...
      <AlertDescription className="text-foreground/50 text-xs">
        {flexible ? (
          ...
          <ul style={{ listStyle: 'outside' }}>
            <li>
              This multisig could be used on Polkadot, Kusama and ALL Substrate
              chains.
            </li>
            <li>
              Once created, the multisig Signers and threshold can NOT be
              modified.
            </li>
          </ul>
        )}
