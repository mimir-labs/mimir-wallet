...
function HideIcon({ address }: { address: string }) {
  ...
  return isHidden ? (
    <Tooltip content='Hidden'>
      <Button size='sm' variant='light' isIconOnly onClick={() => showAccount(address)}>
    ...
    <Tooltip content='Unhide'>
      <Button size='sm' variant='light' isIconOnly className='text-divider-300' onClick={() => hideAccount(address)}>
...
function CreateSuccess({ isOpen, onClose, address }: Props) {
  ...
  return (
    <Modal size='lg' isOpen={isOpen} hideCloseButton>
      <ModalContent>
        <ModalBody className='items-center gap-5 py-5 pt-10'>
          ...
          <div className='bg-secondary flex items-center gap-2.5 self-stretch rounded-[10px] p-2.5'>
          ...
          {meta.isPure &&
            meta.delegatees?.map((delegatee, index) => (
              <div key={index} className='bg-secondary flex items-center gap-2.5 self-stretch rounded-[10px] p-2.5'>
                <div className='bg-success h-[8px] w-[8px] rounded-full' />
