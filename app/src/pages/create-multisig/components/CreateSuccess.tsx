...
function HideIcon({ address }: { address: string }) {
  const { hideAccountHex } = useAccount();
  const addressHex = useMemo(() => addressToHex(address), [address]);
  const isHidden = useMemo(
    () => hideAccountHex.includes(addressHex),
    [hideAccountHex, addressHex],
  );
  ...
  return isHidden ? (
    ...
    <Tooltip content="Unhide">
      <Button size="sm" variant="light" isIconOnly className="text-divider">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="20"
          height="13"
        ...
        <ModalBody className="items-center gap-5 py-5 pt-10">
          ...
          {meta.isPure &&
            meta.delegatees?.map((delegatee, index) => (
              <div
                key={index}
                className="bg-secondary flex items-center gap-2.5 self-stretch rounded-[10px] p-2.5"
              >
                <div className="bg-success h-[8px] w-[8px] rounded-full" />
                <AddressCell
                  className="flex-1"
