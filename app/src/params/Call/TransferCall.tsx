...
const TransferCall = forwardRef<HTMLDivElement | null, CallProps>(
  (props, ref) => {
    const {
      from: propFrom,
      registry,
    ...
    const icon = assetId
      ? assetInfo?.logoUri || findAsset(network, assetId)?.Icon
      : chain.tokenIcon;
        ...
        <div className="text-foreground/50 relative flex flex-1 items-center">
          ...
          >
            {assetId !== null ? (
              ...
              <FormatBalance
                icon={<Avatar className="h-4 w-4" src={icon} />}
                value={value}
                withCurrency
              />
            )}
