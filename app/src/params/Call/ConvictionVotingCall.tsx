...
const ConvictionVotingCall = forwardRef<HTMLDivElement | null, CallProps>(
  (props, ref) => {
    const {
      call,
      className,
      ...
      if (voteInfo.voteType === 'SplitAbstain') {
        return (
          <div className="flex flex-col text-sm">
            ...
            <div>
              Abstain:{' '}
              <FormatBalance value={voteInfo.abstain || '0'} withCurrency />
            </div>
          </div>
        );
      }
    ...
    return (
      <div
        ...
        className={mergeClasses(
          'grid w-full grid-cols-5 items-center gap-4',
          className,
        )}
      >
        ...
        {/* Poll Index */}
        <div className="text-primary col-span-1 text-base font-bold">
          #{voteInfo.pollIndex}
        </div>

        {/* Proposal Title - placeholder as we don't have proposal data */}
        <div className="text-foreground col-span-1 text-center text-base font-bold">
          Referendum Vote
        </div>

        {/* Vote Amount */}
        <div className="text-foreground col-span-1 text-right text-base font-bold">
          {renderVoteAmount()}
        </div>
        ...
        <div className="col-span-1 flex flex-row-reverse">
          <div
            className={`rounded-[20px] px-2.5 py-0.5 ${getVoteColor(voteInfo.vote)}`}
          >
