...
const AddressDisplay = React.memo(
  ({ reverse, address }: { reverse: boolean; address?: string | null }) => {
...
const ChainIcon = React.memo(
  ({
    ...
    chainInfo: CrossChainTransferInfo['destination'];
    side: 'left' | 'right';
  }) => {
...
const CrossChainTransferCall = forwardRef<HTMLDivElement | null, CallProps>(
  (props, ref) => {
    const {
      from: propFrom,
      registry,
    ...
    return (
      ...
      >
        ...
        <div className="text-foreground/50 relative flex flex-1 items-center @max-lg:w-full">
          ...
          <Popover
            onOpenChange={hasMultipleAssets ? toggleOpen : undefined}
            open={open}
          >
            ...
            <PopoverContent className="flex flex-col gap-2.5">
              ...
              {results.assets.map((asset, index) => (
                <div
                  key={index}
                  className="border-divider rounded-[10px] border-1 px-5 py-2.5 text-center"
                >
