  ...
  const peopleNetworks = useMemo(
    () =>
      Array.from(
        new Set(
          endpoints
            .map((network) => network.identityNetwork)
        ...
        {endpoints.map((endpoint) => {
          return (
            <Button
              ...
              style={{
                background: !endpoint.isRelayChain
                  ? 'var(--color-main-bg)'
                  : undefined,
              }}
              onClick={
...
function OmniChainSelect() {
  ...
  const enabledNetworks = useMemo(
    () => chains.filter((network) => network.enabled),
    [chains],
  );
  ...
  const handleEnableNetworks = useCallback(
    (networks: string[]) =>
      networks.forEach((network) => enableNetwork(network)),
    [enableNetwork],
  );

  const handleDisableNetworks = useCallback(
    (networks: string[]) =>
      networks.forEach((network) => disableNetwork(network)),
    [disableNetwork],
  );
  ...
  const triggerButton = (
    ...
    >
      ...
      <span className="block sm:hidden">
        {enabledNetworks.length > 1 ? `${enabledNetworks.length}` : ''}
      </span>
  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        <BorderSpinWrapper loading={!allConnected}>
          {triggerButton}
        </BorderSpinWrapper>
      </PopoverTrigger>
      <PopoverContent
