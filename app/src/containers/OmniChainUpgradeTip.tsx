...
import { cn, Modal, ModalBody, ModalContent, ModalHeader } from '@mimir-wallet/ui';
...
const tabContent: Record<string, { video: string; description: string }> = {
  ...
  quickSimulation: {
    video: 'https://mimir-labs.github.io/mimir-assets/videos/QuickSimulation.mp4',
    description: 'Run a transaction simulation in 1–2 seconds and instantly display changes in account balances.'
  }
};
      ...
      <ModalContent>
        ...
        <ModalBody>
          <div className={cn('flex', upSm ? 'flex-row' : 'flex-col-reverse')}>
            {/* Tab List */}
            <div className={cn('flex gap-1', upSm ? 'flex-col pr-2' : 'flex-row flex-wrap justify-center pt-3')}>
              {tabs.map((tab) => (
                <button
                  key={tab.key}
                  onClick={() => {
                  ...
                  className={cn(
                    ...
                    activeTab === tab.key ? 'bg-secondary text-foreground' : 'hover:text-foreground/70'
                  )}
                >
                  {tab.title}
                </button>
            ...
            {/* Tab Panel */}
            <div className={cn('flex-1 space-y-5', upSm ? 'border-l-secondary border-l pl-3' : '')}>
