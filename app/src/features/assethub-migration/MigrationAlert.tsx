...
function Item({
  ...
  const { isAlertVisible, dismissAlert } = useMigrationStatus(
    sourceChain,
    isCompleted,
  );
  ...
  return (
    <>
      <div
        ...
        className="bg-home-main w-full rounded-[5px]"
      >
          ...
          <div className="relative flex h-[38px] w-full flex-row items-center justify-start gap-2.5 pl-5 text-white">
            ...
            {isCompleted ? (
              <p className="flex-1 text-left text-sm leading-normal font-bold text-white">
                <span>
                  {destNetwork?.name} Migration on {sourceNetwork?.name} has
                  been completed — check out{' '}
                </span>
                <a
                  target="_blank"
                  href={MIGRATION_NOTICE_DOCS_URL}
                ...
                <button
                  className="text-white underline"
                  onClick={handleMigratedAccountsClick}
                >
                  migrated accounts
                </button>
              </p>
              ...
              <p className="flex-1 text-left text-sm leading-normal font-bold text-white">
                ...
                {sourceChain === 'polkadot'
                  ...
                  : sourceChain === 'kusama'
                    ? 'October 7'
                    : 'July 7'}
                . After the migration, all multisig/proxy on{' '}
...
function MigrationAlert({
  ...
  onMigrationCounts: (counts: number) => void;
}) {
