  ...
  return (
    <div className="bg-primary/5 flex items-center gap-2.5 rounded-[10px] p-4">
      <label className="inline-flex cursor-pointer items-center gap-2">
        <Checkbox
          checked={isSelected}
          onCheckedChange={(selected) => onSelect(template.name, !!selected)}
        />
...
function Content({
  ...
  const handleSelectItem = (name: string, selected: boolean) => {
    setSelected((items) =>
      selected ? [...items, name] : items.filter((item) => item !== name),
    );
  };
  ...
  const handleMigrate = () => {
    ...
    selectedTemplates.forEach((template) => {
      try {
        ...
        const destCall = destApi.tx[sourceCall.section][sourceCall.method](
          ...sourceCall.args,
        );
  ...
  return (
    <div className="flex w-full flex-col gap-[15px]">
      ...
      <div className="flex items-center gap-2.5 py-1">
        ...
        <span className="text-foreground text-[18px] font-extrabold whitespace-nowrap">
          {sourceNetwork?.name}
        </span>
        <svg
        ...
        <ModalHeader>
          <div className="flex flex-col gap-2.5">
            <h2 className="text-[20px] font-extrabold">
              Migrate Call Templates
            </h2>
            <p className="text-sm">
              Due to Assethub Migration, some call templates may not work on the
              new chain. You can migrate them to {destNetwork?.name}.
            </p>
          </div>
        </ModalHeader>
