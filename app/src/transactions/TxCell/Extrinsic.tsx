...
function Extrinsic({
  ...
  const { call: parsedCall, isLoading: isParsingCall } =
    useParseCallWithFallback(network, transaction.call, call);
    ...
    <>
      <div className="@container flex flex-1 flex-col gap-2.5">
        <>
          ...
          {hasLargeCalls &&
            (!shouldLoadDetails || (!isFetched && isFetching)) && (
          ...
          <details className="group" open={isMobile}>
            ...
            >
              ...
              {transaction.note && (
                <Item title="Note" content={transaction.note} />
              )}
              {transaction.createdBlock && (
                <Item
                  title="Created Block"
                  content={transaction.createdBlock}
                />
              )}
              <Item
                title="Created Time"
                content={formatDate(transaction.createdAt)}
              />
              {transaction.executedBlock && (
                <Item
                  title="Executed Block"
                  content={transaction.executedBlock}
                />
              )}
          ...
          <div className="flex gap-2.5 pt-2.5">
            <Button asChild variant="ghost">
              <a
                href={
                  transaction.executedExtrinsicHash
                    ? chainLinks.extrinsicExplorerLink(
                        chain,
                        transaction.executedExtrinsicHash,
                      )
                    : chainLinks.extrinsicExplorerLink(
