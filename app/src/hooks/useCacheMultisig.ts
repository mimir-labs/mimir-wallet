...
export function useCacheMultisig(): [data: CacheMultisig[], isLoading: boolean] {
  ...
  const { data, isLoading } = useQuery<CacheMultisig[]>({
    ...
    queryHash: service.getNetworkUrl(`multisig/pending?${accounts.map((address) => `addresses=${address}`).join('&')}`),
    queryKey: [
      accounts.length > 0
        ? service.getNetworkUrl(`multisig/pending?${accounts.map((address) => `addresses=${address}`).join('&')}`)
        : null
    ]
  });
